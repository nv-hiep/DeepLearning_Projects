...
MAX_CONTENT_LENGTH = 2 * 1024 * 1024 # 2MB app.config['MAX_CONTENT_LENGTH'] = 1 * 1024 * 1024
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER  


# load json and create model
json_file = open('model/model_classweight.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)

# load weights into new model
loaded_model.load_weights("model/model_classweight_weight.h5")

# All 24 classes
CLASSES = np.array(['Action', 'Adventure', 'Animation', 'Biography', 'Comedy', 'Crime',
